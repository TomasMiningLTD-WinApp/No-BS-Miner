...
while ($true) {
    ...
    try {
    $DollarRates = Invoke-WebRequest "https://api.fixer.io/latest?base=USD" -UseBasicParsing -TimeoutSec 2 | ConvertFrom-Json | Select-Object -ExpandProperty rates
    $DollarKuna = $DollarRates.HRK
    writelog "Currency conversion API was responsive (USD/HRK=$DollarKuna).........." $logfile $true
    } 
        
    catch {
    writelog "Currency conversion API failed!!!" $logfile $true
    }

    #get actual hour electricity cost KUN
    $CurrentTime = [Int] (Get-Date -UFormat "%H")
 
    if (($CurrentTime -lt 7) -or ($CurrentTime -gt 20)){
            $ElectricityCostValue = 0.58195}

    else{$ElectricityCostValue = 1.06785}

    writelog "Current electricity cost is $ElectricityCostValue HRK." $logfile $true
    $ElectricityCostValue= $ElectricityCostValue / $DollarKuna
    writelog "Current electricity cost is $ElectricityCostValue USD." $logfile $true
